...
const generateLibrary = (books) => {
    ...
    let radioHTML = books.map((book, i) => {
        ...
        return `
            <li>
                <input type="radio" name="author" id="radio${i}" data-id=${i} class="radio-btn" ${(i === randBook || i === 0) ? 'checked' : ''}>
                <label for="radio${i}" class="radio-label">${book.title}</label>
            </li>
        `
    }).join('');

    libraryList.innerHTML = radioHTML;
}

// event delegation for dynamically created radio buttons
libraryList.addEventListener('change', function(e) {
    let target = e.target;
...
copyTextBtn.addEventListener('click', function(e) {
    ...
    let top = coords.top + e.target.offsetHeight + 5;
